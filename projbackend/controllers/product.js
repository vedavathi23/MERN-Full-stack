...
exports.updateProduct = (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        ...
        if(file.photo){
            if(file.photo.size > 3000000){
                return res.status(400).json({
                    error: "Size of the image is too big."
                });
            }
...
exports.deleteProduct = (req, res) => {
    ...
    product.remove((err, deletedProduct) => {
        ...
        return res.json({
            message: "Product has been successfully deleted from DB.",
            deletedProduct
        });
    });
};
...
// Middleware to update the stock and sold
...
exports.updateStock = (req, res, next) => {
    ...
    Product.bulkWrite(myOperations, {}, (err, products) => {
        if(err){
            return res.status(400).json({
                error: "Bulk Operation was failed."
            });
        }
    });
    next();
...
// Fetching Unique Categories from products

exports.getAllUniqueCategories = (req, res) => {
    Product.distinct("category", {}, (err, category) => {
        if(err){
            return res.status(400).json({
                error: "No unique category was found."
            });
        }
